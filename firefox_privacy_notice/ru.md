...
{: #health-report .inproduct-link }
	...
	Цель Отчета о работоспособности Firefox (Firefox Health Report, FHR) — предоставить вам информацию о стабильности и производительности вашего браузера и предложить советы на случай, если у вас возникнут проблемы, например частые сбои или замедленный запуск. Mozilla собирает ваши данные, объединяет их с данными других пользователей Firefox и отправляет обратно в ваш браузер, позволяя вам видеть, как изменяется производительность вашего браузера с течением времени. В этих данных, в частности, указывается тип устройства, операционная система, версия Firefox, дополнения (количество и тип), тайминг событий браузера, визуализация, случаи восстановления сеанса, продолжительность сеанса, взаимодействие с точками доступа к поиску и кодов поисковых партнеров Firefox, давность профиля, количество сбоев и количество страниц. FHR не передает в Mozilla URL-адреса посещаемых вами страниц.
...
{: #telemetry .inproduct-link}
	...
	Эта функция по умолчанию включена в сборках Firefox Nightly и Beta/Developer Edition и помогает их пользователям отправлять отзывы в Mozilla. В основной версии Firefox эта функция по умолчанию отключена.
...
* **Отслеживание направлений и кампаний**{: #thirdparty } ** **{: #referraltracking }

	С целью изучения и совершенствования наших маркетинговых кампаний Firefox по умолчанию передает определенную информацию. К ней относятся «данные направления», такие как домен веб-сайта и рекламная кампания, из которых вы были направлены на загрузку и установку Firefox, а также «данные взаимодействия» о функциях, используемых вами в Firefox.

	__Данные направления__
	В системах Android и iOS Firefox передает данные направления нашему поставщику аналитики для мобильных устройств и прилагает к ним код рекламного объявления Google, IP-адрес, временную метку, название страны, язык, название операционной системы и версию приложения. [Здесь](https://support.mozilla.org/kb/desktop-attribution-privacy) вы сможете больше узнать об этой функции и о том, как отключить ее.

	На настольных компьютерах Firefox записывает и отправляет в Mozilla данные направления в составе Отчета о работоспособности Firefox. [Здесь](https://support.mozilla.org/kb/desktop-attribution-privacy) вы сможете больше узнать об этой функции и о том, как отказаться от нее или отключить ее.

	__Данные взаимодействия__
	В системе iOS Firefox отправляет данные взаимодействия на адрес Leanplum, нашего поставщика услуг мобильного маркетинга, у которого есть собственная [политика приватности](https://www.leanplum.com/privacy/). Эти данные позволяют нам тестировать различные функции и методы работы, а также выдавать специально подобранные сообщения и рекомендации по повышению удобства работы с Firefox. Узнайте больше о [сборе этих данных](https://github.com/mozilla-mobile/firefox-ios/blob/master/MMA.md) и о том, как [отключить эту функцию](https://support.mozilla.org/kb/send-anonymous-usage-data-firefox-mobile-devices).
...
{: #screenshots }
	...
	__Выгрузка скриншотов__
	Скриншоты, выбранные вами для выгрузки, отправляются в Mozilla и сохраняются в течение минимального указанного времени. Продолжительность хранения можно изменить. Мы можем обращаться к выгруженным вами скриншотам, если это разумно необходимо для функционирования службы. Вы можете удалить выгруженные вами скриншоты в любое время. 

	__Данные взаимодействия__
	Мы получаем такие данные, как сведения о посещениях вами веб-сайта «Скриншоты Firefox», частота обращения к выгруженным скриншотам или предоставления к ним доступа вами или другими лицами, а также ваши действия с кнопками, плитками и мышью, связанные с получением скриншотов. 
В отношении посещений веб-сайта «Скриншоты Firefox» типы собираемых нами данных описаны в [уведомлении о приватности наших веб-сайтов](https://www.mozilla.org/privacy/websites/). 

	__Технические данные__
	Мы получаем такие данные, как средний размер и количество выгруженных вами скриншотов, версия вашего браузера Firefox, операционная система устройства и ошибки. IP-адрес, с которого происходило обращение на веб-сайт «Скриншоты Firefox», временно записывается в стандартный журнал сервера. 

	Полная версия документации приведена [здесь](https://github.com/mozilla-services/screenshots/blob/master/docs/METRICS.md), а о том, как удалить все ваши скриншоты, можно прочитать [здесь](https://screenshots.firefox.com/leave-screenshots).

Если не указано иное, настоящее уведомление о приватности распространяется на все последние обычные версии Firefox. Наши предварительные версии (Beta/Developer Edition, Nightly и TestFlight) находятся в процессе активной разработки и могут содержать новые функции или иметь другие характеристики приватности. Из предварительных версий в Mozilla автоматически отправляются [данные телеметрии](https://gecko.readthedocs.io/en/latest/toolkit/components/telemetry/telemetry/index.html), которые помогают нам совершенствовать Firefox.
{: #pre-release }
